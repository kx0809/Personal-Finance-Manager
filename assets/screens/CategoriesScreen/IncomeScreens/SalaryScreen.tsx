...
const SalaryScreen = () => {
  ...
  useEffect(() => {
    const fetchSalaryData = async () => {
      try {
        ...
        const total = salaryData.reduce((sum, item) => sum + parseFloat(item.amount), 0);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Income from salary</Text>
      {data.length === 0 ? (
        <Text style={styles.noData}>No income coming from salary</Text>
      ) : (
        <FlatList
          data={data}
      ...
      <View style={styles.footer}>
        <Text style={styles.totalAmount}>Total Salary: RM {totalAmount.toFixed(2)}</Text>
      </View>
    </View>
  );
};
